...
# x = 0
# y = 0.5 * D2
# while c(x, y) > Ca or np.isnan(c(x, y)) == True and x < 1e4:
#     x = x + 1
# else:
#     print('Lmax =', x ,'m')
...
def Conc_array(x1, x2, y1, y2, inc):
    xaxis = np.arange(x1, x2, inc)
    yaxis = np.arange(y1, y2, inc)
...
#start = timeit.default_timer()
result = Conc_array(-10, 11, -10, 11, inc)
...
plt.xticks(np.arange(20, 171, 50), np.arange(0, 151, 50))
...
for i in range(0,360,1):
    Err.append((c(x_test[i], y_test[i])-Ca)/gamma)
